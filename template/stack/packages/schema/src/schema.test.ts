...
describe('EMBEDDING_DIM', () => {
  it('is 1024 and matches the vector column in the committed migration', () => {
    ...
    expect(migrationSql).toContain(`"embedding" vector(${EMBEDDING_DIM})`);
  });
});
...
describe('DTOs', () => {
  ...
  it('accepts client fields only in NewNoteInput and rejects an empty title', () => {
    const input: NewNote = { title: 'hello', body: 'world' };
    ...
    expect(NewNoteInput.parse({ title: 'x', ownerId: sample.ownerId })).toEqual({ title: 'x' });
  });
...
describe('migration SQL self-check', () => {
  ...
  it('defines all four per-operation owner policies for notes and grants DML to app_api', () => {
    ...
    expect(migrationSql).toContain("(select current_setting('app.user_id', true)::uuid)");
    expect(migrationSql).toContain('GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE "notes" TO "app_api"');
  });
});
